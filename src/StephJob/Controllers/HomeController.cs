namespace StephJob.Controllers
{
    ...
    public class HomeController : Controller
    {
        ...
        public IActionResult Review()
        {
            ...
            model.Industries = unitOfWork.Naics.Find(p => p.Level == 3).Select(p => new { Name = p.Title, Id = p.NaicsId }).Distinct().OrderBy(p => p.Name).Select(p => new SelectListItem { Value = p.Id.ToString(), Text = p.Name });
